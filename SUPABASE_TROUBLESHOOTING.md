# 🔧 Supabase Troubleshooting Guide

Руководство по решению проблем с Supabase интеграцией.

## ❌ Ошибка: "relation 'users' already exists"

### Проблема

```
ERROR: 42P07: relation "users" already exists
```

### Причина

Таблицы уже существуют в Supabase, но схема пытается создать их заново.

### Решение

#### Вариант 1: Использовать безопасную схему (рекомендуется)

1. **В Supabase SQL Editor** выполните содержимое файла:

   ```
   backend/supabase-schema-safe.sql
   ```

2. **Эта схема:**
   - Использует `CREATE TABLE IF NOT EXISTS`
   - Удаляет существующие политики перед созданием новых
   - Обрабатывает конфликты триггеров

#### Вариант 2: Полный сброс (осторожно!)

⚠️ **ВНИМАНИЕ: Это удалит все данные!**

1. **Выполните скрипт сброса:**

   ```
   backend/supabase-reset.sql
   ```

2. **Затем выполните безопасную схему:**
   ```
   backend/supabase-schema-safe.sql
   ```

#### Вариант 3: Ручная очистка

1. **В Supabase Dashboard** → **Table Editor**
2. **Удалите таблицы в порядке:**
   - `subscriptions`
   - `dating_matches`
   - `connections`
   - `charts`
   - `users`

3. **Выполните безопасную схему**

## ❌ Ошибка: "function handle_new_user() already exists"

### Решение

Безопасная схема автоматически заменяет функции:

```sql
CREATE OR REPLACE FUNCTION public.handle_new_user()
```

## ❌ Ошибка: "policy already exists"

### Решение

Безопасная схема удаляет существующие политики:

```sql
DROP POLICY IF EXISTS "Users can view own profile" ON public.users;
```

## ❌ Ошибка подключения к Supabase

### Проверьте переменные окружения

```bash
# Проверьте файл .env
cat backend/.env | grep SUPABASE
```

Должно быть:

```env
SUPABASE_URL="https://your-project.supabase.co"
SUPABASE_ANON_KEY="eyJ..."
SUPABASE_SERVICE_ROLE_KEY="eyJ..."
```

### Проверьте подключение

```bash
# Тест подключения
curl -H "apikey: YOUR_ANON_KEY" \
     -H "Authorization: Bearer YOUR_ANON_KEY" \
     "https://your-project.supabase.co/rest/v1/"
```

## ❌ Ошибка аутентификации

### Проверьте настройки Auth

1. **В Supabase Dashboard** → **Authentication** → **Settings**
2. **Site URL**: `http://localhost:3000`
3. **Redirect URLs**:
   - `http://localhost:3000/api/auth/callback`
   - `http://localhost:8081`

### Проверьте RLS политики

```sql
-- Проверьте активные политики
SELECT schemaname, tablename, policyname, permissive, roles, cmd, qual
FROM pg_policies
WHERE schemaname = 'public';
```

## ❌ Ошибка тестирования

### Проверьте бэкенд

```bash
# Убедитесь, что бэкенд запущен
curl http://localhost:3000/api

# Должен вернуть: "Hello World!"
```

### Проверьте логи

```bash
# В терминале бэкенда должны быть логи:
# ✅ Supabase client initialized
# ✅ Swiss Ephemeris инициализирован
```

## 🔍 Диагностика

### Проверка таблиц

```sql
-- Список всех таблиц
SELECT table_name
FROM information_schema.tables
WHERE table_schema = 'public';
```

### Проверка политик RLS

```sql
-- Список политик
SELECT tablename, policyname, cmd
FROM pg_policies
WHERE schemaname = 'public';
```

### Проверка функций

```sql
-- Список функций
SELECT routine_name
FROM information_schema.routines
WHERE routine_schema = 'public';
```

## 🚀 Быстрое решение

Если ничего не помогает:

1. **Удалите проект Supabase**
2. **Создайте новый проект**
3. **Выполните безопасную схему:**
   ```sql
   -- Скопируйте содержимое backend/supabase-schema-safe.sql
   -- Вставьте в Supabase SQL Editor
   -- Выполните запрос
   ```

## 📞 Получение помощи

### Логи для отладки

```bash
# Запустите тест с подробными логами
node scripts/test-supabase.js 2>&1 | tee supabase-test.log
```

### Проверка статуса

```bash
# Проверьте все компоненты
echo "Backend status:"
curl -s http://localhost:3000/api && echo "✅ Backend OK" || echo "❌ Backend DOWN"

echo "Supabase config:"
grep SUPABASE backend/.env && echo "✅ Config OK" || echo "❌ Config MISSING"
```

## ✅ Успешная настройка

После успешной настройки вы увидите:

```
🧪 Тестирование Supabase интеграции...

1️⃣ Тестирование регистрации через Supabase...
✅ Регистрация успешна!

2️⃣ Тестирование входа через Supabase...
✅ Вход успешен!

3️⃣ Тестирование создания натальной карты...
✅ Натальная карта создана!

4️⃣ Тестирование получения профиля...
✅ Профиль получен!

🎉 Все тесты Supabase прошли успешно!
```
