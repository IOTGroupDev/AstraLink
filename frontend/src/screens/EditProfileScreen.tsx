...
const EditProfileScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const loadData = async () => {
    try {
      ...
      Alert.alert(t('editProfile.alerts.loadFailed.title'), t('editProfile.alerts.loadFailed.message'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddPhoto = async () => {
    if (photos.length >= 20) {
      Alert.alert(t('editProfile.alerts.photoLimit.title'), t('editProfile.alerts.photoLimit.message'));
      return;
    }

    const { status } = await ImagePicker.requestMediaLibraryPermissionsAsync();
    if (status !== 'granted') {
      Alert.alert(t('editProfile.alerts.galleryPermission.title'), t('editProfile.alerts.galleryPermission.message'));
      return;
    }
  ...
  const uploadPhoto = async (uri: string) => {
    try {
      ...
      Alert.alert(t('editProfile.alerts.photoAdded.title'), t('editProfile.alerts.photoAdded.message'));
    } catch (error) {
      logger.error('Error uploading photo', error);
      Alert.alert(t('editProfile.alerts.photoUploadFailed.title'), t('editProfile.alerts.photoUploadFailed.message'));
    } finally {
  ...
  const handleDeletePhoto = async (photoId: string) => {
    Alert.alert(t('editProfile.alerts.deletePhoto.title'), t('editProfile.alerts.deletePhoto.message'), [
      { text: t('common.buttons.cancel'), style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            ...
            Alert.alert(t('editProfile.alerts.photoDeleted.title'), t('editProfile.alerts.photoDeleted.message'));
          } catch (error) {
            Alert.alert(t('editProfile.alerts.photoDeleteFailed.title'), t('editProfile.alerts.photoDeleteFailed.message'));
          }
        },
      },
    ]);
  ...
  const handleSetPrimaryPhoto = async (photoId: string) => {
    try {
      ...
      Alert.alert(t('editProfile.alerts.primaryPhotoSet.title'), t('editProfile.alerts.primaryPhotoSet.message'));
    } catch (error) {
      Alert.alert(t('editProfile.alerts.primaryPhotoFailed.title'), t('editProfile.alerts.primaryPhotoFailed.message'));
    }
  };
    ...
    try {
      ...
      if (address) {
        ...
        Alert.alert(
          ...
          t('editProfile.alerts.locationFound.message', { location: locationString })
        );
      } else {
        Alert.alert(t('editProfile.alerts.locationNotFound.title'), t('editProfile.alerts.locationNotFound.message'));
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      ...
      if (!formData.name?.trim()) {
        Alert.alert(t('editProfile.alerts.nameRequired.title'), t('editProfile.alerts.nameRequired.message'));
        return;
      }

      if (!formData.birthDate) {
        Alert.alert(t('editProfile.alerts.birthDateRequired.title'), t('editProfile.alerts.birthDateRequired.message'));
        return;
      }
      ...
      Alert.alert(t('editProfile.alerts.profileSaved.title'), t('editProfile.alerts.profileSaved.message'), [
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>{t('editProfile.sections.photos')}</Text>
          <Text style={styles.sectionSubtitle}>
            {20 - photos.length} {t('editProfile.sections.photos').toLowerCase()}
          </Text>
              ...
              <View key={photo.id} style={styles.photoCard}>
                ...
                {photo.isPrimary && (
                  <View style={styles.primaryBadge}>
                    ...
                    <Text style={styles.primaryText}>{t('common.buttons.primary')}</Text>
                  </View>
                )}
              ...
              >
                {uploadingPhoto ? (
                  ...
                  <>
                    ...
                    <Text style={styles.addPhotoText}>{t('editProfile.buttons.addPhoto')}</Text>
                  </>
                )}
              </TouchableOpacity>
            )}
        ...
        {/* Basic Info */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>{t('editProfile.sections.basicInfo')}</Text>

          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>{t('editProfile.fields.name')}</Text>
            <AstralInput
              value={formData.name}
          ...
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>{t('editProfile.fields.birthPlace')}</Text>
            <AstralInput
          ...
          <View style={styles.inputGroup}>
            <View style={styles.locationHeader}>
              <Text style={styles.inputLabel}>{t('editProfile.fields.city')}</Text>
              <TouchableOpacity
        ...
        {/* Gender */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>{t('editProfile.fields.gender')}</Text>
        ...
        {/* Bio */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>{t('editProfile.sections.about')}</Text>
        ...
        {/* Interests */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>{t('editProfile.sections.interests')}</Text>
          <Text style={styles.sectionSubtitle}>
          ...
          >
            <Text style={styles.showMoreText}>
              {showAllInterests ? t('editProfile.buttons.showLess') : t('editProfile.buttons.showMore')}
            </Text>
            <Ionicons
          ...
          >
            {saving ? (
              ...
              <>
                ...
                <Text style={styles.saveButtonText}>{t('editProfile.buttons.save')}</Text>
              </>
            )}
          </LinearGradient>
        </TouchableOpacity>
