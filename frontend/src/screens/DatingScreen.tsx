...
export default function DatingScreen() {
  ...
  // ===============================
  const getBadgeLabel = (b?: 'high' | 'medium' | 'low') =>
    b === 'high' ? t('dating.compatibility.high') : b === 'medium' ? t('dating.compatibility.medium') : t('dating.compatibility.low');
  ...
  const handleSendMessage = useCallback(
    async (text: string) => {
      if (!selectedUser?.id) {
        Alert.alert(t('common.errors.generic'), t('dating.errors.userNotSelected'));
        return;
      }
      ...
      try {
        ...
        Alert.alert(t('common.errors.generic'), t('dating.errors.failedToSendMessage'));
      }
    },
