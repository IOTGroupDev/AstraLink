...
//   // Debug helpers
//   useEffect(() => {
//     const cur = candidates[currentIndex];
//     console.log('[Dating] state update: len=', candidates.length, 'idx=', currentIndex, 'current=', cur?.userId);
...
//       try {
//         // 1) Основной источник: быстрые кандидаты через Supabase RPC (бейджи)
//         const data: ApiCandidate[] = (await datingAPI.getCandidates?.(20)) || [];
//         console.log('[Dating] candidates raw count =', Array.isArray(data) ? data.length : 0);
//
//         // 1.1) Для каждой карточки подтягиваем публичный профиль с бэкенда
//         const tasks = (data || []).map(async (candidate) => {
//           try {
//             const profile = await datingAPI.getProfile(candidate.userId);
//             const photo =
...
//               age: (profile.age ?? undefined) as any,
//               zodiacSign: (profile.zodiacSign ?? undefined) as any,
//               bio: profile.bio ?? '',
...
//             return result;
//           } catch (err) {
//             console.log('[Dating] getProfile failed for', candidate.userId, err);
//             const fallbackPhoto =
//               candidate.photoUrl || candidate.avatarUrl || undefined;
//
//             const result: Candidate = {
//               userId: candidate.userId,
//               badge: candidate.badge,
...
//         });
//
//         // Безопасно собираем результаты (не прерываемся из-за одной ошибки)
//         const settled = await Promise.allSettled(tasks);
//         const enrichedData: Candidate[] = settled
//           .map((r) => (r.status === 'fulfilled' ? r.value : null))
//           .filter(Boolean) as Candidate[];
//
//         console.log('[Dating] enriched count =', enrichedData.length, 'first=', enrichedData[0]);
//
...
//           console.log('[Dating] fallback matches count =', Array.isArray(matches) ? matches.length : 0);
...
//
//           {/* Контент */}
//           <View style={[styles.content, { paddingTop: Math.max(insets.top + 8, 40) }]}>
//             {/* Заголовок */}
...
//
//             {/* Карточка */}
//             <View style={{ alignItems: 'center', marginTop: 20, marginBottom: 140 }}>
//               {loadingCards ? (
//                 <View style={{ paddingVertical: 40, alignItems: 'center', justifyContent: 'center' }}>
//                   <ActivityIndicator size="large" color="#8B5CF6" />
//                   <Text style={{ color: 'rgba(255,255,255,0.7)', marginTop: 12 }}>Загрузка карточек...</Text>
//                 </View>
//               ) : !current ? (
//                 <View style={{ alignItems: 'center', justifyContent: 'center', paddingVertical: 40 }}>
//                   <Ionicons name="person-circle-outline" size={64} color="rgba(255,255,255,0.3)" />
