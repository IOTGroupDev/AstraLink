...
const SubscriptionCard: React.FC<SubscriptionCardProps> = ({
  ...
  const tierFeatures: string[] = t(`subscription.tiers.${currentLevel}.features`, { returnObjects: true }) as string[];
      ...
      >
        ...
        <View style={styles.header}>
          ...
          {shouldShowUpgradeButton && onUpgrade && (
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <Text style={styles.upgradeText}>{t('subscription.buttons.upgrade')}</Text>
        ...
        <View style={styles.statusSection}>
          {subscription?.tier === 'free' ? (
            <Text style={styles.statusText}>{t('subscription.status.freeAccount')}</Text>
          ) : (
            <>
              {isExpired ? (
