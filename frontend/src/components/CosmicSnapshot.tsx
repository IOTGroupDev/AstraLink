...
const CosmicSnapshot: React.FC<CosmicSnapshotProps> = ({ connection, onClose }) => {
  const [activeTab, setActiveTab] = useState<'strengths' | 'growth' | 'composite'>('strengths');
  const [aspects, setAspects] = useState([
    { name: 'Солнце в трине к Венере', strength: 85, type: 'harmony' },
    { name: 'Луна в квадрате к Марсу', strength: 45, type: 'challenge' },
    { name: 'Меркурий в соединении с Меркурием', strength: 90, type: 'connection' },
  ]);
  ...
  useEffect(() => {
    orb1Scale.value = withDelay(200, withSpring(1, { damping: 8, stiffness: 100 }));
    orb2Scale.value = withDelay(400, withSpring(1, { damping: 8, stiffness: 100 }));
    connectionLine.value = withDelay(600, withTiming(1, { duration: 1000, easing: Easing.out(Easing.quad) }));
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          {/* 3D Orbs */}
          <Animated.View entering={SlideInUp.delay(400)} style={styles.orbsContainer}>
            <Svg width={width * 0.9} height={200} style={styles.orbsSvg}>
              ...
              <Animated.Circle
                ...
                style={orb1Style}
              />
              <Text x={width * 0.2} y={110} fontSize="16" fill="#fff" textAnchor="middle" fontWeight="bold">
                Вы
              </Text>
              ...
              <Animated.Circle
                ...
                style={orb2Style}
              />
              <Text x={width * 0.7} y={110} fontSize="16" fill="#fff" textAnchor="middle" fontWeight="bold">
                {connection.name}
              </Text>
            </Svg>
          </Animated.View>
          ...
          {/* Categories */}
          <Animated.View entering={SlideInUp.delay(600)} style={styles.categoriesContainer}>
          ...
          {/* Tabs */}
          <Animated.View entering={SlideInUp.delay(800)} style={styles.tabsContainer}>
            ...
            <View style={styles.tabContent}>
              {activeTab === 'strengths' && (
                <View>
                  ...
                  {aspects.filter(a => a.type === 'harmony').map((aspect, index) => (
              ...
              {activeTab === 'growth' && (
                <View>
                  ...
                  {aspects.filter(a => a.type === 'challenge').map((aspect, index) => (
                    <View key={index} style={styles.aspectItem}>
                      <Ionicons name="flash" size={20} color="#EF4444" />
                      <View style={styles.aspectInfo}>
                        <Text style={styles.aspectName}>{aspect.name}</Text>
                        <Text style={styles.aspectDescription}>
                          Этот аспект предлагает возможности для роста и понимания
                        </Text>
                      </View>
                    </View>
                  ))}
                ...
                <View>
                  ...
                  <View style={styles.compositeChart}>
                    <Svg width={200} height={200} style={styles.compositeSvg}>
                      <Defs>
                        <SvgGradient id="compositeGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                          <Stop offset="0%" stopColor="#8B5CF6" stopOpacity="1" />
                  ...
                  <Text style={styles.compositeDescription}>
                    Ваша композитная карта показывает общую энергию отношений и потенциал для совместного роста.
                  </Text>
                </View>
              )}
            </View>
